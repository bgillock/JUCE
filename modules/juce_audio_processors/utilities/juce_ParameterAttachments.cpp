...
TwoValueSliderParameterAttachment::TwoValueSliderParameterAttachment(RangedAudioParameter& param,
...
{
    const ScopedValueSetter<bool> svs(ignoreCallbacks, true);
    valueType == ValueType::MinValue ? slider.setMinValue(newValue, sendNotificationSync) : slider.setMaxValue(newValue, sendNotificationSync);
}

void TwoValueSliderParameterAttachment::sliderValueChanged(Slider*)
{
    if (!ignoreCallbacks)
